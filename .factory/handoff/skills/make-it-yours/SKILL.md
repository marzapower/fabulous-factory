...
two is the template's own reveal (`BuiltOnFactory`, `FeaturesLink` — delete this with
the showcase). The two directories below make up the rest of that showcase — real for
the template repo, convincing a developer evaluating it, not relevant once you have a
real product:

- **The component-docs pages** under `apps/web/app/features/` — one page per template
...
That is the whole technical component set: it exists to explain the template, and once
the pages explaining the template are gone, nothing imports it. Confirm before deleting —
...
act two — the template reveal — and go with the rest of this item.
