...
// Multi-chain layout (preset domains don't ship each other's tables, mirrors
// `packages/db/scripts/migrate.ts`'s shape): the SHARED chain (`user`, etc) lives at
// `@factory/db`'s own migrations folder and runs first; this domain's OWN chain
// (`captures`/`tasks`/`runs`/`run_steps`) lives one level under it, in its own
// subdirectory with its own migrations table, so its history never collides with the
// shared chain's.
const SHARED_MIGRATIONS_FOLDER = path.resolve(__dirname, "../../../db/migrations");
const UNTANGLE_MIGRATIONS_FOLDER = path.resolve(__dirname, "../../../db/migrations/untangle");
const UNTANGLE_MIGRATIONS_TABLE = "__drizzle_migrations_untangle";
...
if (!TEST_DATABASE_URL) {
  console.warn(
    "integration tests skipped: set TEST_DATABASE_URL to run packages/untangle tasks integration tests against a real Postgres instance.",
  );
}
...
describe.skipIf(!TEST_DATABASE_URL)(
  ...
  () => {
    ...
    // database (same key convention as packages/db/llm/core/untangle's other integration
    ...
    beforeEach(async () => {
      ...
      await migrate(db, {
        migrationsFolder: UNTANGLE_MIGRATIONS_FOLDER,
        migrationsTable: UNTANGLE_MIGRATIONS_TABLE,
      });
