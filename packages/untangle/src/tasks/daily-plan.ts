...
import { DailyPlanTemplate } from "../email/daily-plan";
...
/** Subject line for the daily-plan email — owned here (not `@factory/email`'s `SUBJECTS`
 * map) since the template itself now lives in this domain package; `sendRendered` takes
 * the subject as a caller-supplied argument rather than looking one up by template name. */
const DAILY_PLAN_SUBJECT = "Your plan for today";
...
export const notifyStep: RunStep<DailyPlanState> = {
  ...
  async run(state, ctx) {
    ...
    const result = await sendRendered({
      to: email,
      subject: DAILY_PLAN_SUBJECT,
      react: DailyPlanTemplate({
    ...
    if (!result.delivered && result.reason !== "console") {
      console.warn("[@factory/untangle] daily-plan email not delivered:", result.reason);
    }
