 ...
 * `packages/untangle/src/tasks/` is code adopters KEEP and build on, where an exported
 ...
 * the call site, because this file already owns every task/capture query. `user` comes
 * from `@factory/db/schema` (the shared, cross-domain table) — this domain's own
 * `../schema` barrel exports only `captures`/`tasks`/`runs`/`run_steps`. */
export async function getUserEmail(userId: string): Promise<string | undefined> {
  const [row] = await getDb().select({ email: user.email }).from(user).where(eq(user.id, userId));
  return row?.email;
}
