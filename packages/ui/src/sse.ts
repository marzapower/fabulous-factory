/**
 * Client-side SSE frame parser (plan K.8.4). Deliberately pure and side-effect free — no
 * `fetch`, no DOM, nothing that can't run under a plain `node` test environment — which is
 * exactly why this logic lives here instead of inline in a React reader loop: it is the
 * only part of the transport that can be unit-tested at all.
 *
 * The server side of this contract writes one frame per event as
