...
1. **`pnpm factory:status`** — always exits 0; this is a report, not a gate. Renders
   `LAUNCH.md` from the repo root: the stage line, one line per checklist item (`✓` done,
   `○` open — annotated ` — blocks launch` and ` → skill: <skill>` where applicable, `🔒`
   shown on sign-off items), and a closing
   `<done>/<total> done · <open blockers> launch blocker(s) open` count. No `LAUNCH.md`
   at root prints "no LAUNCH.md found — nothing to report" instead.
...
3. **`pnpm preflight`** — the mechanical gate, stage-aware from `.factory/config.json`'s
   ...
   `STRIPE_SECRET_KEY` still starting with `sk_test_`, and `CLAUDE.md` / `AGENTS.md` no
   longer containing the literal pointer `docs/agents/conventions.md`. One warning is
   non-blocking at **every** stage, by
