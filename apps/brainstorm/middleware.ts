...
// turn route) stay gated — everything below is deliberately public. EXACT entries, not
// prefixes, for a route with no sub-paths of its own (H.10-style review fix, mirrored
// from apps/untangle/middleware.ts): a prefix entry would also allowlist any same-prefix
// sibling route added later for free.
const EXACT_ALLOWLIST = new Set([
  ...
  "/features",
  // Live examples backing the /features docs pages — exact, not prefix, for the same
  // reason documented above: neither route has sub-paths of its own, and a prefix entry
  // would allowlist any same-prefix sibling added later for free.
