...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "app.meta.terms" });
...
export default async function TermsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const tt = await getTranslations("app.legal.terms");
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-12">
      ...
      <div className="mt-8 space-y-8 text-sm leading-relaxed text-muted-foreground">
        <section>
          <h2 className="text-xl font-semibold text-foreground">{tt("section1.title")}</h2>
          <p className="mt-2">{tt("section1.body")}</p>
        </section>

        <section>
          <h2 className="text-xl font-semibold text-foreground">{tt("section2.title")}</h2>
          <p className="mt-2">{tt("section2.body")}</p>
        </section>

        <section>
          <h2 className="text-xl font-semibold text-foreground">{tt("section3.title")}</h2>
          <p className="mt-2">{tt("section3.body")}</p>
        </section>

        <section>
          <h2 className="text-xl font-semibold text-foreground">{tt("section4.title")}</h2>
          <p className="mt-2">{tt("section4.body")}</p>
        </section>

        <section>
          <h2 className="text-xl font-semibold text-foreground">{tt("section5.title")}</h2>
          <p className="mt-2">{tt("section5.body")}</p>
        </section>
      </div>
