 ...
 *
 * `readonly RunEvent[]`, typed against the real engine (`@factory/untangle`), is the whole
 * point of N5: if `packages/untangle/src/runs/engine.ts`'s event shape ever changes, this
 ...
 * (`packages/untangle/src/runs/engine.ts`) and `capturePipeline`'s real step keys/labels
 * (`packages/untangle/src/tasks/pipeline.ts`) exactly — three steps, `extract` → `triage` →
 ...
 * `packages/untangle/src/tasks/heuristics.ts`'s `locateQuote` uses for the real pipeline. */
