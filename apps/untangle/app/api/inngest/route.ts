...
import { inngest, functions as productFunctions } from "@factory/jobs";
// Untangle's own cron functions ship with the preset (separate from the generic
// registry above, which is populated by the adopter's own jobs via `pnpm gen job`).
import { functions as untangleFunctions } from "@factory/untangle";
...
export const { GET, POST, PUT } = serve({
  client: inngest,
  functions: [...productFunctions, ...untangleFunctions],
});
