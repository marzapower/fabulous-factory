...
// config), so it's legal here for the same reason.
...
// One static page per declared locale (i18n plan §2.3) — `dynamicParams = false` means
// any other locale segment 404s instead of falling through to a fresh render.
export const generateStaticParams = () => generateLocaleParams(i18n);
export const dynamicParams = false;

/**
 * The PRODUCT's metadata, not the template's — `/` is Fabulous Nothing's own landing
 * page and the browser tab has to agree with it. The template gets its own title on
 * `/features`, and `make-it-yours` points an adopter here as one of the strings to
 * rename.
 */
export async function generateMetadata({
  ...
  const t = await getTranslations("app.meta.layout");
